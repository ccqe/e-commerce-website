...
import {matchSubSrc,matchGoods} from './data'

export default class MatchBar extends Component{
	...
	render(){
		return(
            ...
            <div className="wiring con_width clearfix">
				<h1 className="list_title">搭配
					<ul>
						<li
						className={`${this.state.currentContent === 0 ? "active" : ""}`}
						onMouseOver={()=>this.changeContent(0)}><a href="#">热门</a></li>
						<li
						className={`${this.state.currentContent === 1 ? "active" : ""}`}
						onMouseOver={()=>this.changeContent(1)}><a href="#">耳机音箱</a></li>
					</ul>
				</h1>
				<div className="wiring_left">
					<a href="#"><img src={matchSubSrc[0]}/></a>
					<a href="#"><img src={matchSubSrc[1]}/></a>
				</div>
				<div className="wiring_right">		
                    {
                        matchGoods.map((item,index)=>{
                            return(
